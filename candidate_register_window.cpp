...
CandidateRegisterWindow::CandidateRegisterWindow(QWidget *parent) : QWidget(parent)
{
    ...
    //title
    title = new QLabel("Candidate Registratrion", this);
    ...
    grid->addWidget(pass_label, 5, 0);
    grid->addWidget(pass_input, 5, 1);
    grid->addWidget(pass_warn,  5, 2);

    grid->addWidget(manifesto_label, 6, 0);
    grid->addWidget(manifesto_input, 6, 1);
    grid->addWidget(manifesto_warn,  6, 2);

    grid->addWidget(msg, 7, 0, 1, 3);

    grid->addWidget(reg_btn, 8, 0, 1, 3);

    grid->addWidget(back_btn, 9, 0, 1, 1);
...
void CandidateRegisterWindow::register_user()
{
    ...
    if(f_empty || l_empty || n_empty || p_empty || m_empty|| pty_empty) return;
